...
const schematic : (PartNumber | null)[][] = inputLines.map(line => new Array(line.length).fill(null));
...
let sum = 0;
const partNumberSet = new Set<PartNumber>();
for (let y = 0; y < schematic.length; y ++) {
    const line = schematic[y];
    for (let x = 0; x < line.length; x++) {
...
function getAt(x: number, y: number) : PartNumber | null {
